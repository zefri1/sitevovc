...
class MarketplaceCategoryNavigator {
  ...
  generateCategoryItemHTML(data){
    ...
    const escapedName = escapeHtmlAttr(data.name || '');
    const escapedNameText = escapeHtml(data.name || '');
    let html = `<div class="category-item ${hasChildren?'has-children':''}" data-category="${escapedName}">
      <div class="category-main">
        <span class="category-name">${escapedNameText}</span>
    ...
    if(hasChildren){
      ...
      data.children.forEach(child=>{
        ...
        seen.add(child.name);
        const escapedChildName = escapeHtmlAttr(child.name || '');
        const escapedChildNameText = escapeHtml(child.name || '');
        html += `<div class="category-child" data-category="${escapedChildName}">
          <span class="category-name">${escapedChildNameText}</span>
