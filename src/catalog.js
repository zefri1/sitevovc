  ...
  renderProducts() {
    ...
    const renderChunk = () => { 
      ...
      const html = slice.map((p, idx) => { 
        ...
        return `<article class="product-card" data-product-id="${escapedId}" style="animation-delay: ${animationDelay}s;"><div class="product-image">${img}${ph}</div><div class="product-info"><h3 class="product-name">${escapedName}</h3><p class="product-brand">${escapedBrand}</p><p class="product-color">${escapedColor}</p>${escapedSize ? `<span class="product-card__size">${escapedSize}</span>` : ''}<div class="product-price">${p.price.toLocaleString('ru-RU')} ₽</div><div class="product-status">${badge}</div></div></article>`; 
      }).join(''); 
...
// Убрана дублирующая инициализация - теперь только в catalog-enhanced.js
// Если этот файл импортируется отдельно, можно раскомментировать:
// let catalog = null;
